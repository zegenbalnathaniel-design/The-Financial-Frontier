...
import { INDICATORS, FAMILIES, currentValue, series, colorForIndicator, fmt } from "@/lib/indicators";
...
export default function GlobeSection() {
  ...
  const chart = useMemo(() => {
    if (!selected) return [];
    return series(selected, ind, 24).map((v, i) => ({ i, v }));
  }, [selected, ind]);
  const up = selected ? selected.data.change >= 0 : true;
    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <div className="glass relative h-[30rem] overflow-hidden sm:h-[34rem]">
          ...
          <Globe ind={ind} onSelect={setSelected} />
              ...
              <motion.div key={selected.code} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
                ...
                <div className="mt-5 flex items-baseline gap-2">
                  ...
                  <span className={`inline-flex items-center gap-1 font-mono text-sm ${up ? "text-pos" : "text-neg"}`}>
                    {up ? <TrendingUp size={14} /> : <TrendingDown size={14} />}{up ? "+" : ""}{selected.data.change}%
                  </span>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chart}>
                      <defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor={colorForIndicator(ind, currentValue(selected, ind))} stopOpacity={0.5} />
                        <stop offset="100%" stopColor={colorForIndicator(ind, currentValue(selected, ind))} stopOpacity={0} />
                      </linearGradient></defs>
                      ...
                      <Area type="monotone" dataKey="v" stroke={colorForIndicator(ind, currentValue(selected, ind))} strokeWidth={1.5} fill="url(#g)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2.5">
                  {PANEL_KEYS.map((k) => {
                    ...
                    const v = currentValue(selected, mi);
                    return (
                      <div key={k} className="rounded-xl border border-white/8 bg-white/[0.02] p-3">
                        <div className="font-display text-base" style={{ color: colorForIndicator(mi, v) }}>{fmt(v, mi)}</div>
                ...
                <p className="mt-4 flex items-start gap-2 font-mono text-[10px] leading-relaxed text-muted2">
                  ...
                  {selected.sourced ? "Growth/inflation/rate grounded in our June 2026 report; other metrics & charts illustrative." : "Illustrative sample data for the visualisation."}
                </p>
              </motion.div>
            ) : (
